...
export function sanitizeMessages(value) {
  ...
  const messages = value.slice(-MAX_MESSAGES)
    ...
    .map((message) => ({ role: message.role, content: typeof message.content === "string" ? message.content.trim().slice(0, MAX_MESSAGE_CHARS) : "" }))
...
export default async function handler(request, response) {
  ...
  if (!apiKey) return response.status(503).json({ error: "StreamVista AI configuration is temporarily unavailable." });
  const messages = sanitizeMessages(request.body?.messages);
  if (!messages) return response.status(400).json({ error: "A valid conversation is required." });
  ...
  try {
    const upstream = await fetch(endpoint, {
      ...
      body: JSON.stringify({ contents: toGeminiContents(messages), systemInstruction: { parts: [{ text: SYSTEM_INSTRUCTION }] }, generationConfig: { maxOutputTokens: 1200 } }),
    });
    const payload = await upstream.json().catch(() => null);
    if (!upstream.ok) return response.status(upstream.status === 429 ? 429 : 502).json({ error: upstream.status === 429 ? "StreamVista AI quota is temporarily unavailable." : "StreamVista AI is temporarily unavailable." });
    const reply = extractReply(payload);
    if (!reply) return response.status(502).json({ error: "StreamVista AI returned an empty response." });
    return response.status(200).json({ reply });
  } catch (error) {
    console.error("StreamVista AI request failed", error instanceof Error ? error.message : "unknown");
