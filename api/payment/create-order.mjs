...
export default async function handler(request, response) {
  ...
  try {
    ...
    const { data: deal, error: dealError } = await client.from("sv_marketplace_deals").select("id,buyer_id,title_id,status,contract_status,price,payment_status").eq("id", dealId).eq("buyer_id", user.id).maybeSingle();
    if (dealError) throw dealError;
    if (!deal) return json(response, 404, { error: "Deal not found" });
    if (!["approved","payment_pending","requested"].includes(String(deal.status || "").toLowerCase())) return json(response, 409, { error: "Deal is not ready for payment" });
    ...
    const amount = integerPaise(deal.price);
    const currency = safeCurrency("INR");
    ...
    const { data: payment, error: insertError } = await client.from("sv_payments").insert({ user_id:user.id,title_id:deal.title_id,deal_id:deal.id,purpose:"marketplace_deal",provider:"razorpay",provider_order_id:payload.id,amount:deal.price,currency,status:"created",idempotency_key:idempotencyKey }).select("id,provider_order_id,amount,currency,status,purpose,created_at").single();
