...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (!secret || !verifyCheckoutSignature(secret, orderId, paymentId, signature)) return json(response, 400, { error: "Invalid payment signature" });
    const { data: payment, error: paymentError } = await client.from("sv_payments").select("id,user_id,deal_id,provider_order_id,provider_payment_id,amount,currency,status").eq("provider","razorpay").eq("provider_order_id",orderId).eq("user_id",user.id).maybeSingle();
    if (paymentError) throw paymentError;
    if (!payment) return json(response, 404, { error: "Payment order not found" });
    const upstream = await fetch(`https://api.razorpay.com/v1/payments/${encodeURIComponent(paymentId)}`, { headers: { Authorization: razorpayAuth() } });
    ...
    const { data: updated, error: updateError } = await client.from("sv_payments").update({ provider_payment_id:paymentId,status:nextStatus,verified_at:new Date().toISOString() }).eq("id",payment.id).select("id,provider_order_id,provider_payment_id,amount,currency,status").single();
    if (updateError) throw updateError;
    if (payment.deal_id && nextStatus === "captured") await client.from("sv_marketplace_deals").update({ payment_status:"paid" }).eq("id",payment.deal_id);
