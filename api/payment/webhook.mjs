...
export default async function handler(request, response) {
  ...
  try {
    ...
    const { error: eventInsertError } = await client
      .from("sv_payment_webhook_events")
      .insert({ event_id: eventId, event_name: eventName, payload_hash: hash, status: "received" });
    if (eventInsertError && eventInsertError.code !== "23505") throw eventInsertError;

    const payment = eventPayment(payload);
    ...
    if (["payment.captured", "payment.authorized", "payment.failed"].includes(eventName) && payment?.id) {
      ...
      const update = {
        ...
        provider_order_id: payment.order_id || undefined,
      ...
      if (data?.deal_id) {
        await client
          ...
          .update({ payment_status: nextStatus === "captured" ? "paid" : nextStatus })
          .eq("id", data.deal_id);
      }
    ...
    const { error: eventUpdateError } = await client
      .from("sv_payment_webhook_events")
      .update({ status: "processed", processed_at: new Date().toISOString(), payment_id: paymentId })
      .eq("event_id", eventId);
    ...
    if (client && eventId) {
      await client.from("sv_payment_webhook_events").update({ status: "failed" }).eq("event_id", eventId).catch(() => {});
    }
