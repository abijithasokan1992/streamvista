...
export async function verifyPayment(req: Request, res: Response) {
  try {
    ...
    const idempotencyKey = headerKey || `${orderId}:${paymentId}`;
    const db = admin();
    ...
    const { data, error } = existing
      ? await db.from('sv_payments').update(row).eq('id', existing.id).select('id,status,verified_at').single()
      : await db.from('sv_payments').insert(row).select('id,status,verified_at').single();
