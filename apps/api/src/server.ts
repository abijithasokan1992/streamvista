...
async function startServer() {
  ...
  app.post("/api/payments/verify", authenticateToken, async (req: any, res) => {
    ...
    if (!orderId || !paymentId || !signature) return res.status(400).json({ error: "Missing verification parameters." });
    const isValid = paymentService.verifySignature(orderId, paymentId, signature);
    if (isValid) {
      res.json({ success: true, message: "Payment verified successfully.", certificateUrl: `/api/assets/${assetId}/clearance-certificate` });
    } else {
  ...
  app.post("/api/signup", async (req, res) => {
    ...
    if (!fullName || !email || !password || !targetWorkspace) return res.status(400).json({ error: "Missing required security parameters." });
    let connection;
    try {
      ...
      if (!connection) {
        ...
        memoryUsers.push({ userId: Date.now(), fullName, email, passwordHash: hashedPassword, workspace: targetWorkspace, role: 'user', isActive: 1 });
        return res.json({ success: true, message: "Workspace allocation node initialized.", metadata: { identityAppId: OCI_IDENTITY_APP_ID, targetWorkspace, proxyStatus: "SIMULATED_DB_COMMIT" } });
      }
      await connection.execute(`INSERT INTO CRAYONS_USERS (FULL_NAME, EMAIL, PASSWORD_HASH, WORKSPACE) VALUES (:name, :email, :password, :workspace)`, { name: fullName, email, password: hashedPassword, workspace: targetWorkspace }, { autoCommit: true });
      ...
      if (err.errorNum === 1) return res.status(400).json({ error: "Email already exists in the system." });
  ...
  app.get("/api/police/intelligence", authenticateToken, async (req: any, res) => { const news = await intelligenceService.getLatestIntelligence(); res.json({ success: true, intelligence: news }); });
  app.get("/api/police/cases/public", authenticateToken, async (req: any, res) => { const cases = await intelligenceService.getPublicCases(); res.json({ success: true, cases }); });
  app.post("/api/police/investigation/simulate", authenticateToken, async (req: any, res) => {
    ...
    if (suspect) { suspect.financialActivity = finLogs; suspect.locationHistory = locLogs; }
  ...
  app.get("/api/police/suspects", authenticateToken, async (_req, res) => { res.json({ success: true, suspects: memorySuspects }); });
...
startServer().then((app) => { app.listen(3000, () => console.log("StreamVista API listening on :3000")); });
