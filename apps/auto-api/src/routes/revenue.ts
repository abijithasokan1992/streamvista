...
async function listRevenue(req: any, res: any) {
  ...
  try {
    ...
    res.json({ success: true, currency: rows[0]?.currency || 'INR', grossRevenue: gross, transactions: rows, payments: rows, count: rows.length });
  } catch (error: any) {
