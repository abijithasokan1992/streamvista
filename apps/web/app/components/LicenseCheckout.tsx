...
const LicenseCheckout: React.FC<LicenseCheckoutProps> = ({ assetId, title, price, onSuccess, onClose }) => {
  ...
  const handlePayment = async () => {
    ...
    try {
      const orderRes = await axios.post('/api/payments/create-order', { amount: price, assetId }, { headers: { Authorization: `Bearer ${token}` } });
      ...
      const paymentObject = new window.Razorpay({
        ...
        handler: async (response: any) => {
          try {
            const verifyRes = await axios.post('/api/payments/verify', { razorpay_order_id: order.id, razorpay_payment_id: response.razorpay_payment_id, razorpay_signature: response.razorpay_signature, assetId }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    } catch (err) { console.error(err); setError('Could not initiate payment. Please try again.'); setStatus('ERROR'); setLoading(false); }
  };
