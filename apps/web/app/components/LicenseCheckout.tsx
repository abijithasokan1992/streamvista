...
const LicenseCheckout: React.FC<LicenseCheckoutProps> = ({ assetId, title, price, onSuccess, onClose }) => {
  ...
  const handlePayment = async () => {
    ...
    try {
      const idempotencyKey = `sv_${assetId.replace(/[^A-Za-z0-9]/g, '').slice(0, 40)}_${crypto.randomUUID()}`;
      ...
      const order = orderRes.data?.payment;
      const razorpay = orderRes.data?.razorpay;
      if (!order?.provider_order_id || !razorpay?.keyId) throw new Error('Payment order was not created');
      ...
      const options = {
        ...
        handler: async (response: any) => {
          try {
            ...
            if (verifyRes.data?.payment?.status === 'captured') {
              setStatus('SUCCESS');
              onSuccess();
            } else {
              setError('Payment was received but is not yet captured.');
              setStatus('ERROR');
            }
          } catch (verificationError: any) {
            setError(verificationError?.response?.data?.error || 'Payment verification failed.');
            setStatus('ERROR');
          }
        },
      ...
      setError(paymentError?.response?.data?.error || paymentError?.message || 'Could not initiate payment.');
      setStatus('ERROR');
      setLoading(false);
    }
  };
  ...
  return (
    <div className="checkout-overlay">
      <div className="checkout-modal">
        {status === 'IDLE' && <div className="checkout-content">
          <h2>Secure Licensing</h2>
          <div className="checkout-summary">
            <div className="summary-item"><span>Asset</span><span>{title}</span></div>
            <div className="summary-item"><span>Title ID</span><span className="mono">{assetId}</span></div>
            <div className="summary-item total"><span>Total Amount</span><span>₹{price.toLocaleString('en-IN')}</span></div>
          </div>
          <p className="legal-note">Payment is processed by Razorpay. Your license transaction is recorded against the authenticated buyer and marketplace deal.</p>
          <div className="checkout-actions">
            <button className="cancel-btn" onClick={onClose}>Cancel</button>
            <button className="proceed-btn" onClick={handlePayment} disabled={loading}>{loading ? <Loader2 className="animate-spin" /> : 'Proceed to Payment'}</button>
          </div>
        </div>}
        {status === 'PROCESSING' && <div className="status-view"><Loader2 className="animate-spin w-12 h-12" /><p>Initializing secure transaction…</p></div>}
