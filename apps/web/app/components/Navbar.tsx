  ...
  return (
    <nav className="navbar" aria-label="Primary">
      ...
      {isOpen && <div className="mobile-menu">
        <Link to="/dashboard" onClick={close}>Workspace</Link><Link to="/film-os" onClick={close}>Projects</Link><Link to="/crayons-bridge" onClick={close}>Bridge</Link><Link to="/revenue" onClick={close}>Revenue</Link><Link to={isAuthenticated ? '/profile' : '/login'} onClick={close}>{isAuthenticated ? 'Account' : 'Sign in'}</Link>{isAuthenticated && <button type="button" onClick={handleLogout}>Log out</button>}
      </div>}
      <style>{`
