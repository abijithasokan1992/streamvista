...
type Project = { id: string; name: string; logline: string | null; stage: string; approval_state: string; organization_id?: string };
...
export default function FilmOS() {
  ...
  useEffect(() => { const load = async () => { if (!supabase) { setProjectsError('Supabase is not configured.'); setLoading(false); return; } const { data, error } = await supabase.from('film_projects').select('id,name,logline,stage,approval_state,organization_id').order('created_at', { ascending: false }).limit(50); if (error) { setProjectsError(error.message); setLoading(false); return; } const list = (data || []) as Project[]; setProjects(list); setProject(id ? list.find((p) => p.id === id) || null : list[0] || null); setLoading(false); }; void load(); }, [id]);
  useEffect(() => { const loadRows = async () => { if (!supabase || !projectId) return; const table = tab === 'scripts' ? 'scripts' : tab === 'scenes' ? 'scenes' : tab === 'shots' ? 'shots' : tab === 'assets' ? 'assets' : 'approvals'; let query = supabase.from(table).select('*').limit(50); if (table === 'approvals') query = query.eq('project_id', projectId); else query = query.eq('project_id', projectId); const { data, error } = await query; if (!error) setRows((data || []) as Row[]); }; void loadRows(); }, [projectId, tab]);

  const createProject = async (event: React.FormEvent) => {
    event.preventDefault(); setNotice(''); if (!supabase || !newName.trim()) return;
    const { data: { user } } = await supabase.auth.getUser(); if (!user) { setNotice('Sign in required.'); return; }
    setBusy(true);
    let { data: org } = await supabase.from('organizations').select('id,name').eq('owner_id', user.id).order('created_at', { ascending: true }).limit(1).maybeSingle();
    if (!org) {
      const { data: createdOrg, error: orgError } = await supabase.from('organizations').insert({ name: `${(user.email || 'My')} Studio`, owner_id: user.id }).select('id,name').single();
      if (orgError) { setBusy(false); setNotice(`Organization setup failed: ${orgError.message}`); return; }
      org = createdOrg;
      const { error: membershipError } = await supabase.from('memberships').insert({ organization_id: org.id, user_id: user.id, role: 'owner' });
      if (membershipError) { setBusy(false); setNotice(`Organization created but membership setup failed: ${membershipError.message}`); return; }
    }
    const { data, error } = await supabase.from('film_projects').insert({ organization_id: org.id, name: newName.trim(), created_by: user.id, stage: 'development', approval_state: 'draft' }).select().single();
    ...
    setBusy(false); if (memberError) { setNotice(`Project created but owner membership failed: ${memberError.message}`); return; }
  ...
  return <div className="min-h-screen bg-[#050607] text-zinc-100"><header className="sticky top-0 z-30 border-b border-white/10 bg-[#050607]/90 backdrop-blur-xl"><div className="mx-auto flex max-w-[1500px] items-center justify-between px-5 py-4"><div><Link to="/" className="text-xs font-semibold tracking-[0.32em] text-white/60">STREAMVISTA</Link><div className="mt-1 text-lg font-semibold">Film OS</div></div><div className="flex items-center gap-3 text-xs text-white/45"><span>{project?.name || 'No project selected'}</span><Link className="rounded-full border border-white/10 px-3 py-2 hover:bg-white/5" to="/">Home</Link></div></div></header>
    <div className="mx-auto grid max-w-[1500px] gap-6 px-5 py-6 lg:grid-cols-[250px_1fr]"><aside className="rounded-2xl border border-white/10 bg-white/[0.025] p-4"><div className="text-xs font-semibold tracking-[0.2em] text-white/30">PROJECTS</div><form onSubmit={createProject} className="mt-4 space-y-2"><input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="New film / project" className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none focus:border-white/25" /><button disabled={busy} className="w-full rounded-xl bg-white px-3 py-2 text-sm font-semibold text-black">{busy ? 'Creating…' : 'Create project'}</button></form><div className="mt-5 space-y-1">{projects.map((p) => <button key={p.id} onClick={() => setProject(p)} className={`w-full rounded-xl px-3 py-3 text-left text-sm ${project?.id === p.id ? 'bg-white/10 text-white' : 'text-white/55 hover:bg-white/5'}`}>{p.name}</button>)}{projects.length === 0 && !projectsError && <div className="px-3 py-3 text-xs text-white/30">No Film OS projects yet.</div>}</div></aside>
      <main className="space-y-5">{projectsError && <div className="rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-3 text-sm text-red-300">{projectsError}</div>}{notice && <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white/65">{notice}</div>}
        {!project ? <section className="rounded-2xl border border-white/10 bg-white/[0.025] p-8"><h1 className="text-3xl font-semibold">Build the next film.</h1><p className="mt-2 max-w-xl text-sm leading-6 text-white/40">Create a project to connect script, scenes, shots, assets, AI runs, approvals, edit, localization, QC and delivery in one production graph.</p></section> : <><section className="rounded-2xl border border-white/10 bg-white/[0.025] p-6"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-xs tracking-[0.2em] text-white/30">ACTIVE PROJECT</div><h1 className="mt-2 text-3xl font-semibold">{project.name}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-white/40">{project.logline || 'Idea → Script → Production → Post → Localization → QC → Master → Buyer → Revenue'}</p></div><div className="rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-right"><div className="text-xs text-white/30">STATE</div><div className="mt-1 text-sm font-semibold">{project.approval_state.replace('_', ' ').toUpperCase()}</div></div></div><div className="mt-6 grid grid-cols-2 gap-2 sm:grid-cols-4 lg:grid-cols-11">{departments.map(([key,label],index) => <button key={key} onClick={() => setTab(memberTabs[key] || key)} className={`rounded-lg border px-2 py-2 text-[10px] ${index <= stageIndex ? 'border-white/20 bg-white/10 text-white' : 'border-white/5 bg-white/[0.02] text-white/30'}`}>{label}</button>)}</div></section>
