...
import { generateOAuthState, validateOAuthState, generatePKCE, encryptToken, decryptToken } from './security';
...
export interface StoredIntegrationRecord {
  ...
  connectionStatus: 'not_connected' | 'connecting' | 'connected' | 'permission_incomplete' | 'token_expiring' | 'authorization_revoked' | 'sync_failed';
...
const workspaceStore = new Map<string, StoredIntegrationRecord>();
...
export class InstagramController {
   ...
   */
  static async handleConnect(workspaceId: string) {
    if (!workspaceId) {
      throw {
    ...
    const authUrl = `https://www.facebook.com/${META_API_VERSION}/dialog/oauth?client_id=${META_APP_ID}&redirect_uri=${encodeURIComponent(
      OAUTH_REDIRECT_URI
    )}&state=${state}&scope=${scopeList}&response_type=code&code_challenge=${codeChallenge}&code_challenge_method=S256`;
   ...
   */
  static async handleCallback(code: string, state: string, workspaceId: string) {
    if (!code) {
      throw {
    ...
    if (!state || !validateOAuthState(state, workspaceId)) {
      throw {
        code: 'INVALID_STATE',
    ...
    const encryptedAccessToken = encryptToken(simulatedAccessToken);
   ...
   */
  static async handleGetAccount(workspaceId: string) {
   ...
   */
  static async handleGetMedia(workspaceId: string, limit: number = 25) {
    const record = workspaceStore.get(workspaceId);
    ...
    decryptToken(record.encryptedAccessToken);
   ...
   */
  static async handleGetInsights(workspaceId: string) {
    const record = workspaceStore.get(workspaceId);
    if (!record || record.connectionStatus !== 'connected') {
      throw {
        code: 'EXPIRED_TOKEN',
   ...
   */
  static async handleGetComments(workspaceId: string, mediaId: string) {
    const record = workspaceStore.get(workspaceId);
    if (!record || record.connectionStatus !== 'connected') {
      throw {
        code: 'EXPIRED_TOKEN',
   ...
   */
  static async handleRefresh(workspaceId: string) {
    const record = workspaceStore.get(workspaceId);
    if (!record) {
      throw {
        code: 'EXPIRED_TOKEN',
   ...
   */
  static async handleDisconnect(workspaceId: string) {
