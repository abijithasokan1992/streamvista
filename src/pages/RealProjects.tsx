...
const projects = [
  {
    ...
    description: "A completed StreamVista/Crayons Pictures film project with a documented theatrical release and production-company record.",
    source: "https://en.wikipedia.org/wiki/Kolumittayi",
    cta: "Discuss rights / distribution",
  },
  {
    ...
    cta: "Enquire about distribution",
  },
  {
    ...
    cta: "Request Telugu rights enquiry",
  },
];
          ...
          {projects.map((project) => (
            <article key={project.title} className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.04] shadow-2xl">
              <div className="aspect-[2/3] overflow-hidden bg-slate-900">
                <img src={project.poster} alt={`${project.title} official promotional poster`} className="h-full w-full object-cover" loading="lazy" />
              </div>
              <div className="p-5">
                ...
                <div className="mt-5 grid grid-cols-1 gap-2">
                  <Link to={`/chat?project=${encodeURIComponent(project.title)}&intent=enquiry`} className="inline-flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-950 hover:bg-violet-100"><MessageCircle size={16} /> Enquire</Link>
                  <Link to={`/chat?project=${encodeURIComponent(project.title)}&intent=licensing`} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/5 px-4 py-3 text-sm font-bold text-white hover:bg-white/10"><Handshake size={16} /> Request licensing</Link>
                  <Link to={`/chat?project=${encodeURIComponent(project.title)}&intent=sales-distribution`} className="inline-flex items-center justify-center gap-2 rounded-xl border border-violet-400/30 bg-violet-500/10 px-4 py-3 text-sm font-bold text-violet-200 hover:bg-violet-500/20"><ShieldCheck size={16} /> Sales & distribution</Link>
                </div>
