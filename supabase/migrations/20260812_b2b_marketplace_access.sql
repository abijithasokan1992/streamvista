...
-- Enables buyer discovery, screening requests, licensing deals, and title-scoped private film storage.
...
create table if not exists public.sv_screening_requests (
  ...
  buyer_id uuid not null references public.sv_app_profiles(id),
  title_id uuid not null references public.sv_app_titles(id),
...
create policy sv_screening_admin_update on public.sv_screening_requests for update to authenticated using ((select private.sv_app_is_admin())) with check ((select private.sv_app_is_admin()));
...
-- Media paths are {title_id}/{filename}. The title row is created before upload so ownership can be verified.
...
create policy sv_films_secure_read on storage.objects for select to authenticated using (
  ...
  and (
    ...
    or (select private.sv_app_is_admin())
  )
);
