...
-- This migration establishes the minimum real data plane used by the current app:
-- Auth profile/RBAC, titles, rights, screening/deals, payments, webhook ledger and private media.
...
create trigger on_auth_user_created_streamvista
after insert on auth.users
for each row execute function public.handle_streamvista_auth_user();
...
set search_path = public, pg_catalog
as $$
  select coalesce(to_jsonb(p), '{}'::jsonb) from public.sv_app_profiles p where p.id = auth.uid();
$$;

create or replace function public.sv_admin_profiles()
returns setof public.sv_app_profiles
language sql
stable
security definer
set search_path = public, pg_catalog
as $$
  select p from public.sv_app_profiles p
  where public.sv_current_role() in ('founder','super_admin','admin');
$$;
...
revoke all on function public.sv_current_role() from public, anon;
grant execute on function public.sv_current_role() to authenticated, service_role;
...
revoke all on function public.sv_admin_profiles() from public, anon;
grant execute on function public.sv_admin_profiles() to authenticated, service_role;
...
revoke all on function public.is_command_admin() from public, anon;
grant execute on function public.is_command_admin() to authenticated, service_role;
...
create policy sv_profiles_update_own_safe on public.sv_app_profiles for update to authenticated using (id = auth.uid()) with check (id = auth.uid() and app_role = (select app_role from public.sv_app_profiles where id = auth.uid()) and verification_status = (select verification_status from public.sv_app_profiles where id = auth.uid()));
...
create policy sv_films_creator_insert on storage.objects for insert to authenticated with check (
  bucket_id='streamvista-films' and exists (
    select 1 from public.sv_app_titles t where t.id::text=(storage.foldername(name))[1] and (t.creator_id=auth.uid() or private.sv_app_is_admin())
  )
);
drop policy if exists sv_films_secure_read on storage.objects;
create policy sv_films_secure_read on storage.objects for select to authenticated using (
  bucket_id='streamvista-films' and (
    exists (select 1 from public.sv_app_titles t where t.id::text=(storage.foldername(name))[1] and t.creator_id=auth.uid())
    or exists (select 1 from public.sv_screening_requests s where s.buyer_id=auth.uid() and s.status='approved' and s.title_id::text=(storage.foldername(name))[1])
    or private.sv_app_is_admin()
  )
);
drop policy if exists sv_films_creator_update on storage.objects;
create policy sv_films_creator_update on storage.objects for update to authenticated using (
  bucket_id='streamvista-films' and exists (select 1 from public.sv_app_titles t where t.id::text=(storage.foldername(name))[1] and (t.creator_id=auth.uid() or private.sv_app_is_admin()))
);
drop policy if exists sv_films_creator_delete on storage.objects;
create policy sv_films_creator_delete on storage.objects for delete to authenticated using (
  bucket_id='streamvista-films' and exists (select 1 from public.sv_app_titles t where t.id::text=(storage.foldername(name))[1] and (t.creator_id=auth.uid() or private.sv_app_is_admin()))
);
