...
create table if not exists public.sv_app_titles (
  ...
  title text not null default 'Untitled', synopsis text, content_type text, primary_language text, director text,
  status text not null default 'draft' check (status in ('draft','qc','approved','ready_for_distribution','archived')),
  commercial_profile jsonb not null default '{}'::jsonb, metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);

create table if not exists public.sv_title_rights (
  id uuid primary key default gen_random_uuid(), title_id uuid not null references public.sv_app_titles(id) on delete cascade,
  owner_id uuid not null references public.sv_app_profiles(id) on delete cascade, licensed_buyer_id uuid references public.sv_app_profiles(id) on delete set null,
  territory text, platform text, exclusivity text, licensing_model text not null default 'non-exclusive', rights_start_date date, rights_end_date date,
  created_at timestamptz not null default now()
);

create table if not exists public.sv_screening_requests (
  id uuid primary key default gen_random_uuid(), buyer_id uuid not null references public.sv_app_profiles(id) on delete cascade,
  ...
  created_at timestamptz not null default now(), unique (buyer_id,title_id)
);

create table if not exists public.sv_marketplace_deals (
  id uuid primary key default gen_random_uuid(), buyer_id uuid not null references public.sv_app_profiles(id) on delete cascade,
  seller_id uuid references public.sv_app_profiles(id) on delete set null, title_id uuid not null references public.sv_app_titles(id) on delete cascade,
  status text not null default 'requested', contract_status text not null default 'pending', payment_status text not null default 'unpaid',
  price numeric not null default 0, revenue_split numeric not null default 70 check (revenue_split between 0 and 100),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);

create table if not exists public.sv_payments (
  id uuid primary key default gen_random_uuid(), user_id uuid references public.sv_app_profiles(id) on delete set null,
  title_id uuid references public.sv_app_titles(id) on delete set null, deal_id uuid references public.sv_marketplace_deals(id) on delete set null,
  provider text not null default 'razorpay', provider_order_id text, provider_payment_id text, provider_event_id text,
  amount numeric not null default 0, currency text not null default 'INR', purpose text not null default 'streamvista',
  status text not null default 'created' check (status in ('created','authorized','captured','verified','failed','refunded')),
  error_reason text, idempotency_key text unique, raw_event_hash text, verified_at timestamptz not null default now(),
  created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);

create table if not exists public.sv_payment_webhook_events (
  event_id text primary key, provider_event_id text, event_name text not null, payload_hash text not null,
  status text not null default 'received' check (status in ('received','processed','failed')), payment_id text,
  received_at timestamptz not null default now(), processed_at timestamptz
);

create table if not exists public.ai_jobs (
  id uuid primary key default gen_random_uuid(),
  project_id uuid, asset_id uuid, tool text not null, provider text,
  status text not null default 'queued' check (status in ('queued','running','completed','failed','cancelled')),
  progress integer not null default 0 check (progress between 0 and 100),
  input jsonb not null default '{}'::jsonb, output jsonb,
  error text, cost numeric, created_by uuid references auth.users(id) on delete set null,
...
create index if not exists idx_ai_jobs_project on public.ai_jobs(project_id);

create or replace function public.handle_streamvista_auth_user()
returns trigger language plpgsql security definer set search_path = public, pg_catalog as $$
begin
  insert into public.sv_app_profiles (id,email,display_name) values (new.id,coalesce(new.email,''),coalesce(new.raw_user_meta_data->>'display_name',''))
  on conflict (id) do update set email=excluded.email,
    display_name=case when excluded.display_name<>'' then excluded.display_name else public.sv_app_profiles.display_name end, updated_at=now();
  return new;
end; $$;
...
create or replace function public.sv_current_role() returns text language sql stable security definer set search_path = public, pg_catalog as $$
  select coalesce((select app_role from public.sv_app_profiles where id=auth.uid()),'creator_partner'); $$;
create or replace function public.sv_session_profile() returns jsonb language sql stable security definer set search_path = public, pg_catalog as $$
  select coalesce((select to_jsonb(p) from public.sv_app_profiles p where p.id=auth.uid()),'{}'::jsonb); $$;
create or replace function public.sv_admin_profiles() returns setof public.sv_app_profiles language sql stable security definer set search_path = public, pg_catalog as $$
  select p from public.sv_app_profiles p where public.sv_current_role() in ('founder','super_admin','admin'); $$;
create or replace function private.sv_app_is_admin() returns boolean language sql stable security definer set search_path = public, private, pg_temp as $$
  select public.sv_current_role() in ('founder','super_admin','admin'); $$;
create or replace function private.sv_buyer_verified() returns boolean language sql stable security definer set search_path = public, private, pg_temp as $$
...
create or replace function private.sv_title_is_approved(p_status text) returns boolean language sql immutable as $$ select p_status in ('approved','ready_for_distribution'); $$;
create or replace function public.is_command_admin() returns boolean language sql stable security definer set search_path = public, pg_catalog as $$
  select public.sv_current_role() in ('founder','super_admin','admin'); $$;
...
create policy if not exists sv_ai_jobs_read_own on public.ai_jobs for select to authenticated using (created_by=auth.uid() or private.sv_app_is_admin());
create policy if not exists sv_ai_jobs_insert_own on public.ai_jobs for insert to authenticated with check (created_by=auth.uid());
create policy if not exists sv_ai_jobs_update_own on public.ai_jobs for update to authenticated using (created_by=auth.uid() or private.sv_app_is_admin()) with check (created_by=auth.uid() or private.sv_app_is_admin());
