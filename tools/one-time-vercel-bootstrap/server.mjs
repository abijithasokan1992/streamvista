...
function validatedAnonKeyFromText(text) {
  ...
  for (const candidate of new Set(candidates)) {
    ...
    if (payload?.ref === EXPECTED_SUPABASE_PROJECT_REF && payload?.role === 'anon') {
      return candidate;
    }
  }
  return null;
...
async function discoverPublishableKey() {
  ...
  if (override) {
    ...
    if (payload?.ref !== EXPECTED_SUPABASE_PROJECT_REF || payload?.role !== 'anon') {
      throw new Error('Supabase publishable-key override failed canonical project/role validation. No mutation was attempted.');
    }
    return override;
  }
  ...
  if (!homeResponse.ok) {
    throw new Error(`Could not read working StreamVista client shell (HTTP ${homeResponse.status}). No mutation was attempted.`);
  }
  const html = await homeResponse.text();
  const inline = validatedAnonKeyFromText(html);
  if (inline) return inline;

  const assetPaths = [...html.matchAll(/<script[^>]+src=["']([^"']+\.js(?:\?[^"']*)?)["']/gi)]
    .map((match) => match[1])
    .slice(0, 12);

  if (assetPaths.length === 0) {
    throw new Error('Working StreamVista client exposed no JavaScript bundle for publishable-key discovery. No mutation was attempted.');
  }
...
async function runBootstrap() {
  ...
  return {
    ...
    redeploy: {
      ...
      url: deployment.url || redeploy.url,
    },
    readiness,
    credentialsPersistedByBootstrap: false,
  };
}
  ...
  try {
    ...
    if (id !== undefined) {
      send({
        ...
        id,
        result: {
          content: resultText({ ok: false, error: error instanceof Error ? error.message : String(error) }),
          isError: true,
        },
      });
    }
...
if (process.argv.includes('--direct')) {
  ...
  rl.on('line', async (line) => {
    ...
    try {
      ...
      // Keep the one-time MCP server alive on malformed JSON input.
    }
  });
}
